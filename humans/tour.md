...
**흥미로운 지점** — 텍스트를 두 벌로 렌더링합니다. 볼 때는 Konva Text(캔버스 안,
DOM 없음), 편집할 때는 Tiptap(HTML 오버레이)입니다. 둘의 padding·lineHeight 상수가
1px만 어긋나도 편집을 시작하는 순간 글자가 튑니다. 그래서 상수를 `constants/`에
...
**흥미로운 지점** — 리듬게임에서 가장 중요한 규칙 하나: **시간축은 하나입니다.**
