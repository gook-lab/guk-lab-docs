...
| `kind` | `node`·`godot` 등 프로젝트 실행 환경. 생략하면 `node` |
...
| `portfolioSources` | 포트폴리오 프로젝트 링크를 대조할 한·영 콘텐츠 파일 |
...
CI처럼 형제 프로젝트가 없는 환경에서는 저장소 지도만 검사하고 데모·포트폴리오
원문 검사는 `SKIP`으로 표시합니다. 로컬 프로젝트가 있는데 링크가 다르면 `sync`가
실패 코드로 종료합니다.
