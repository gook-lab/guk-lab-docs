...
bottling(Malt Index)은 이 순서를 끝까지 통과했습니다(2026-09-07, PR #65 · #68 ·
#70 · #72 · #73 · #74). 거기서 굳은 규칙 세 가지입니다.

- **어댑터가 정상 종착지일 수 있습니다.** 앱의 계약(예: Button 의
  disabledReason, 정지 스켈레톤)이 코어보다 넓으면, 코어를 부풀리지 말고
  앱 쪽 얇은 어댑터가 그 계약을 지킵니다. 콜사이트는 바뀌지 않습니다.
- **겉 전환은 테마 블록이 합니다.** 앱 CSS 를 지우기 전에 그 값(실측 대비
  포함)을 theme-malt-components.css 로 옮기고, 스크린샷 diff 로 증명한 뒤
  지웁니다. bottling 은 Button 겉 전환이 픽셀 diff 0 이었습니다.
- **남기는 것도 결정입니다.** 옮기지 않는 컴포넌트는 이유를 소스
  (ui-web index.ts)에 적습니다 — 카피가 내용인 것, 앱 어포던스,
  사용처 1곳짜리.
